...
def news_detail(request,word):
    ...
    shownews = News.objects.filter(pk=id)
    ...
    code = News.objects.get(pk=id).pk  ## For Commnet Section
    ...
    link = "/urls/" + str(News.objects.get(pk=id).rand)  ## For QR Code
...
def news_add(request):
    ...
    cat = SubCat.objects.all() ### Categories (to get all the category) ###
