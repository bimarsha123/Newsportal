...
def category(request):
    ...
    news = News.objects.all()  ## for reverse(ordering) need to filter by pk with (-) to get the latest submission first.

    cat = Cat.objects.all()  ## Show categories in footer
    category = Cat.objects.filter(id=query).first()  ## Show categories in footer
    ...
    return render(request, 'front/category.html', {'category':category,'site':site, 'news':news, 'cat':cat, 'subcat':subcat, 'lastnews':lastnews, 'popnews2':popnews2, 'trending':trending})
##--#--## About Page(about) Function For Front (User Interface - Frontend) End ##--#--##
